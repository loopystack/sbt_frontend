...
const WalletManagement: React.FC = () => {
  ...
  const fetchSweepSummary = async () => {
    try {
      const response = await fetch('http://62.169.28.113:8000/api/wallet/sweep-summary');
      if (response.ok) {
  ...
  const fetchPendingDeposits = async () => {
    try {
      const response = await fetch('http://62.169.28.113:8000/api/wallet/pending-deposits?limit=20');
      if (response.ok) {
        const data = await response.json();
  ...
  const triggerSweep = async (asset?: string, network?: string) => {
    ...
    try {
      let url = 'http://62.169.28.113:8000/api/wallet/sweep-all';
      if (asset && network) {
        url = `http://62.169.28.113:8000/api/wallet/sweep/${asset}/${network}`;
      }
